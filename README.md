...
```
usage: gcexport.py [-h] [--version] [-v] [--username USERNAME] [--password PASSWORD] [--what {activities,courses}]
                   [-c COUNT] [-sd START_DATE] [-ed END_DATE] [-e EXTERNAL] [-a ARGS]
                   [-f {gpx,tcx,original,json}] [-d DIRECTORY] [-s SUBDIR] [-lp LOGPATH]
                   [-u] [-ot] [--desc [DESC]] [-t TEMPLATE] [-fp] [-si START_INDEX_NO]
...
optional arguments:
  ...
  -sd START_DATE, --start_date START_DATE
                        the start date to get activities or courses from (inclusive). Format example: 2023-07-31
  -ed END_DATE, --end_date END_DATE
                        the end date to get activities or courses to (inclusive). Format example: 2023-07-31
  ...
  -si START_INDEX_NO, --start_index_no START_INDEX_NO
                        give index for first activity or course to import, i.e. skipping the newest activities or 
                        courses
...
The format of the CSV export file can be customized with template files (in Properties format, see the `--template` option); three examples for activities are included:
